 ...
 * ----------------------------------------------------
 *
 * $Id: masterpayment_sofortbanking.php 04.06.2012 - 22:43 $
 *	
 *	The Modul based on:
...
require_once(DIR_FS_CATALOG . 'includes/external/masterpayment/MasterpaymentActions.class.php');
  ...
  class masterpayment_sofortbanking extends MasterpaymentActions {
    var $code, $title, $description, $enabled;

    function __construct() {
      global $order;

      $this->code		 	= 'masterpayment_sofortbanking';
      $this->title 			= 'Masterpayment ('.MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_TEXT_TITLE.')';
	  $this->title_checkout = MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_TEXT_TITLE;
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_ZONE > 0) ) {
        $check_flag = false;
        $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function get_error() {
       $error = array('title' => MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_TEXT_ERROR_HEADING,
                     'error' => MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_TEXT_ERROR_MESSAGE);
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_STATUS'");        
    ...
    function keys() {
      return array('MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_STATUS', 'MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_ALLOWED', 'MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_ICON', 'MODULE_PAYMENT_MASTERPAYMENT_SOFORTBANKING_SORT_ORDER');
    }
