 ...
 * ----------------------------------------------------
 *
 * $Id: masterpayment_debit_card.php 08.11.2012 01:06 $
 *	
 *	The Modul based on:
...
require_once(DIR_FS_CATALOG . 'includes/external/masterpayment/MasterpaymentActions.class.php');
  ...
  class masterpayment_debit_card extends MasterpaymentActions {
    ...
    function __construct() {
      ...
      $this->title 			= 'Masterpayment ('.MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_TEXT_TITLE.')';
	  $this->title_checkout = MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_TEXT_TITLE;
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_ZONE > 0) ) {
        $check_flag = false;
        $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {	  
	  	if(MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_ICON == 'True') {	  
			...
			$content = array_merge($content, array (
				array (
					'title' => ' ',
					'field' => xtc_image(DIR_WS_ICONS. 'masterpayment_debit_card.gif')
				)
			));			
		}	  
    ...
    function get_error() {
       $error = array('title' => MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_TEXT_ERROR_HEADING,
                     'error' => MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_TEXT_ERROR_MESSAGE);
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_STATUS'");        
    ...
    function keys() {
      return array('MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_STATUS', 'MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_ALLOWED', 'MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_ICON', 'MODULE_PAYMENT_MASTERPAYMENT_DEBIT_CARD_SORT_ORDER');
    }
